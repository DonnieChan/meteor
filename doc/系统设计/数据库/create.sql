...
create table instance_data_delay
(
   ttime                datetime not null comment '时间',
   source_task_id       int not null comment '源头任务ID',
   task_id              int not null comment '任务id',
   delay_millis         int not null comment '延迟毫秒数',
   create_time          timestamp not null default CURRENT_TIMESTAMP comment '创建时间',
   primary key (ttime, source_task_id, task_id),
   key(create_time)
) comment = '数据延迟明细'
engine = InnoDB
default charset utf8;
...
create table alarm_loop_compare
(
   source_task_id       int not null comment '源头任务ID',
   reduce_rate          double not null comment '环比下降比率',
   alarm_gap_min        int not null comment '报警最小间隔',
   alarm_users          varchar(200) not null comment '报警用户列表，英文逗号分隔',
   remarks              text comment '备注',
   is_valid             tinyint not null default 1 comment '是否还可用，1可用，0已下线不可用',
   create_time          timestamp not null default CURRENT_TIMESTAMP comment '创建时间',
   update_time          datetime comment '更新时间',
...
create table alarm_slice_delay
(
   source_task_id       int not null comment '源头任务ID',
   delay_second         int not null comment '延迟秒数',
   alarm_gap_min        int not null comment '报警最小间隔',
   alarm_users          varchar(200) not null comment '报警用户列表，英文逗号分隔',
   remarks              text comment '备注',
   is_valid             tinyint not null default 1 comment '是否还可用，1可用，0已下线不可用',

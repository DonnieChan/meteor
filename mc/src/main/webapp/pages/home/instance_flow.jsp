...
<!-- topbar ends -->
	...
	<div class="container-fluid">
			<div class="row-fluid">
				...
				<form name="searhForm" id="searchForm" method="post" action="${ctx}/instanceFlow.do">		
					<div class="form-inline" style="margin-bottom:20px;">
						<input name="pageNumber" id="pageNumber" type="hidden" />
						<input id="sourceTaskId" name="sourceTaskId" type="hidden" value="${instanceFlowQuery.sourceTaskId}" />
						<label class="control-label">起止时间：</label>
						<input type="text" class="Wdate" size="12" id="startTime" name="startTime" onClick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${instanceFlowQuery.startTime}"/> - 
						<input type="text" class="Wdate" size="12" id="endTime" name="endTime" onClick="WdatePicker({isShowClear:false,dateFmt:'yyyy-MM-dd HH:mm:ss'})" value="${instanceFlowQuery.endTime}"/>				
				...
				<table class="table table-bordered sortable">
				  ...
				  <tbody>
				  	...
				  	<c:forEach var="item" items="${instanceFlows}"  varStatus="idx">
				  		<tr>
				  		  <td>${status+1}</td>
				  		  <td>${item.status}</td>
				  		  <td>
					      ...
					      <td><fmt:formatDate value="${item.initTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					      <td><fmt:formatDate value="${item.startTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					      <td><fmt:formatDate value="${item.endTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					      <td><a href="${ctx}/instanceFlowTask.do?instanceFlowId=${item.instanceFlowId}&duration=${item.endTime.time-item.startTime.time}">${item.instanceFlowId}</a></td>
					    </tr>

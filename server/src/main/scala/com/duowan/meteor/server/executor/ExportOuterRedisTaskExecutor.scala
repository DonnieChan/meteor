...
class ExportOuterRedisTaskExecutor extends AbstractTaskExecutor with Logging {
  ...
  override def exec(instanceTaskExecutor: InstanceTaskExecutor, paramMap: Map[String, Any]): Unit = {
    ...
    if (StringUtils.startsWith(fetchSql, "csql_group_by_1:")) {
      ...
      execForeachPartition(fetchSql, fileId, host, port, password, redisScript, sourceTaskId)
    } else {
      ...
      sendRedis(df, fileId, host, port, password, redisScript, sourceTaskId)
      DropTableUtil.dropTable(targetTable)
    }
  }

  def sendRedis(df: DataFrame, fileId: Integer, host: String, port: Int, password: String, redisScript: String, sourceTaskId: Integer): Unit = {
   ...
   *
   */
  def execForeachPartition(sql: String, fileId: Integer, host: String, port: Int, password: String, redisScript: String, sourceTaskId: Integer): Unit = {
